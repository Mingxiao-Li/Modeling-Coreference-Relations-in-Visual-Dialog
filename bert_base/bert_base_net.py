...
class VDNetEmbedding(nn.Module):
    ...
    def forward(self, input_txt, input_img, img_loc, token_type_ids):
        ...
        c_txt_embeddings = word_embedings + position_embeddings + token_type_embeddings
...
class VDNet(nn.Module):
    ...
    def forward(
        ...
        output_h = self.embeddings(input_txt, input_imgs, img_loc, token_type_ids)
...
class VDNetPredictionHead(nn.Module):
    ...
    def forward(self, output_h):
        ...
        return img_output, lm_output, nsp_output
...
class VDModel(nn.Module):
    ...
    def forward(
        ...
        outputs = self.vd_net(
            input_txt, input_img, img_loc, token_type_ids, padding_mask=padding_mask
        )

        output_h = outputs[0]

        img_out, lm_out, nsp_score = self.vd_prediction_head(output_h)
