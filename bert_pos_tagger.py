...
class PosSentTagger(object):
    ...
    def __init__(self,pos_tag_file):
        if not os.path.exists(pos_tag_file):
            raise ValueError("Can't find a pos_tag file at path {}".format(pos_tag_file))

        self.tag_dic = self.load_post_tag(pos_tag_file)
        self.id_to_tag = collections.OrderedDict([(ids,tag) for tag,ids in self.tag_dic.items()])
...
if __name__ == "__main__":
    word_list = ['[CLS]','the','large','clay',"[SEP]",'jar','is','beside','a','staircase','[MASK]','can','you','tell','if','there','is','anything','inside','the','jar','?','[MASK]','i','can',"'",'t','tell','[PAD]']
    ...
    print(t.encode(['he', 'she', 'it', 'their', '.', 'you', 'are', 'there', 'people', 'on', 'the', 'sidewalks']))
    print(t.get_target(['he', 'she', 'it', 'their', '.', 'you', 'are', 'there', 'people', 'on', 'the', 'sidewalks']))
    print(t.detec_pron(['he', 'she', 'it', 'their', '.', 'you', 'are', 'there', 'people', 'on', 'the', 'sidewalks']))
