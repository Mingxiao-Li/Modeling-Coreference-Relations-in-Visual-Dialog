...
train_info = open("bert_pos_img_language", "a+")
...
if __name__ == "__main__":
    ...
    for epoch_id, idx, batch in batch_iter(train_dataloader, params):
        ...
        if iter_id % 100 == 0 and iter_id > 0:
            ...
            print_format = "[%s][Ep: %.2f][Iter: %d][Time: %5.2fs][NSP + LM Loss: %.3g][LM Loss: %.3g][NSP Loss: %.3g][POS Loss: %.3g][IMG Loss: %.3g]"
            print_info = [
                timestamp,
                cur_epoch,
                iter_id,
