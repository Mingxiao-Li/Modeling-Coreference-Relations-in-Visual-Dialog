...
from bert_pos_tagger import PosSentTagger
...
class VDDataset(data.Dataset):
    ...
    def __init__(self,params):
        ...
        pos_tagger = PosSentTagger(params["pos_dictionary"])
    ...
    def encode_input(self,utterances,MASK,CLS,SEP,max_seq_len,mask_prob):
        ...
        pos_tag_target_list.append(31)
        pos_tag_list.append(31)
        pro_index_list.append(0)
        segment_id_list.append(0)

        for i,cur_utterance in enumerate(utterances):
            word_list = self.tokenizer.convert_ids_to_tokens(cur_utterance)
            pos_tag = self.pos_tagger.encode(word_list)
            pos_target = self.pos_tagger.get_target(word_list)

            if i > 0 and i < len(utterances) - 2:
                pron_index = self.pos_tagger.detec_pron(word_list)
                ...
                pron_index = [0] * len(word_list)

            masked_token_list.extend(cur_masked_index)
            token_id_list.extend(cur_utterance)
            pos_tag_target_list.extend(pos_target)
            pos_tag_list.extend(pos_tag)
            pro_index_list.extend(pron_index)
        ...
        assert len(masked_token_list) == len(token_id_list) == len(pos_tag_target_list) == len(pos_tag_list) == len(pro_index_list)
        ...
        pos_tag_target_list = list2tensorpad(pos_tag_target_list,max_seq_len)
        pos_tag_list = list2tensorpad(pos_tag_list,max_seq_len)
        pro_index_list = list2tensorpad(pro_index_list,max_seq_len)

        if mask_prob != 0.0:
            masked_tokens[0,pro_index_list[0,:]==1] = 1
        masked_tokens[0,masked_tokens[0,:] == 0] = -1
        mask = masked_tokens[0,:] == 1
        masked_tokens[0,mask] = tokens[0,mask]
        tokens[0,mask] = MASK
        pos_tag_list[0,mask] = 0
        pos_tag_target_list[0,masked_tokens[0,:]==-1] = -1

        return tokens,segments,masked_tokens,padding_mask,pos_tag_list,pos_tag_target_list

    def encode_img(self,features,num_boxes,boxes,image_target,mask_prob,max_regions=37):
        # process image
        output_label = []
    ...
    def __getitem__(self,index):
        ...
        if self._split == "train":
            ...
            for j,context in enumerate(utterances):
                ...
                tokens,segment,mask,padding_mask,pos_tag_ids,pos_tag_target_list = self.encode_input(context,self.MASK,self.CLS,self.SEP,max_seq_len=MAX_SEQ_LEN,mask_prob=self.params["mask_prob"])
                tokens_all.append(tokens)
                masked_all.append(mask)
                padding_mask_all.append(padding_mask)
                segments_all.append(segment)
                pos_tag_ids_all.append(pos_tag_ids)
                pos_tag_target_all.append(pos_tag_target_list)
                ...
                for context_random in utterances_random[j]:
                    ...
                    tokens_random,segment, mask_random,padding_mask,pos_tag_ids,pos_tag_target_list = self.encode_input(context_random,self.MASK,self.CLS,self.SEP,max_seq_len=MAX_SEQ_LEN,mask_prob=self.params["mask_prob"])
                ...
                pos_tag_ids_all_rnd.append(torch.cat(pos_tag_ids_all,0).unsqueeze(0))
                pos_tag_target_all_rnd.append(torch.cat(pos_tag_target_all,0).unsqueeze(0))
            ...
            pos_tag_ids_all_rnd = torch.cat(pos_tag_ids_all_rnd,0)
            pos_tag_target_all_rnd = torch.cat(pos_tag_target_all_rnd,0)
            ...
            item["pos_tag_ids"] = pos_tag_ids_all_rnd
            item["pos_tag_target"] = pos_tag_target_all_rnd

            # read image feature
            features, num_boxes, boxes, _, image_target = self._image_features_reader[img_id]
            features,img_loc,image_label,image_target,image_mask = self.encode_img(features,num_boxes,boxes,image_target,mask_prob=self.params["mask_prob"])
            ...
            for rnd,cur_rnd_options in enumerate(options_all):
                ...
                for i,cur_rnd_option in enumerate(cur_rnd_options):
                    ...
                    tokens,segments,mask,padding_mask,pos_tag_list,pos_tag_target = self.encode_input(cur_rnd_option,self.MASK,self.CLS,self.SEP,max_seq_len=MAX_SEQ_LEN,mask_prob=0)
                    ...
                    pos_tag_ids_all_rnd.append(pos_tag_list)
                    pos_tag_target_all_rnd.append(pos_tag_target)
                ...
                pos_tag_ids_all.append(torch.cat(pos_tag_ids_all_rnd,0).unsqueeze(0))
                pos_tag_target_all.append(torch.cat(pos_tag_target_all_rnd,0).unsqueeze(0))
            ...
            pos_tag_ids_all = torch.cat(pos_tag_ids_all,0)
            pos_tag_target_all = torch.cat(pos_tag_target_all,0)
            ...
            item["pos_tag_ids"] = pos_tag_ids_all
            item["pos_tag_target"] = pos_tag_target_all
            ...
            for j,option in enumerate(options_all):
                ...
                tokens, segments, mask,padding_mask,pos_tag_ids_list, pos_tag_target= self.encode_input(option, self.MASK,self.CLS,self.SEP,max_seq_len=MAX_SEQ_LEN,mask_prob=0)

                tokens_all.append(tokens)
                mask_all.append(mask)
                segments_all.append(segments)
                padding_mask_all.append(padding_mask)
                pos_tag_ids_all.append(pos_tag_ids_list)
                pos_tag_target_all.append(pos_tag_target)
            ...
            item["pos_tag_ids"] = pos_tag_ids_all.unsqueeze(0)
            item["pos_tag_target"] = pos_tag_target_all.unsqueeze(0)
