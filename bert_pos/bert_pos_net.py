...
class VDAttention(nn.Module):
    ...
    def forward(self, h, padding_mask):
        ...
        attn_vec = self.self(h, h, h, padding_mask=padding_mask)

        attn_vec_h = attn_vec[0]
        if self.output_attentions:
            attn_vec_h, attn_prob_h = attn_vec
...
class VDNetEmbedding(nn.Module):
    ...
    def forward(self, input_txt, pos_tag, input_img, img_loc, token_type_ids):
...
class VDNet(nn.Module):
    ...
    def forward(
        ...
        output_h = self.embeddings(
            input_txt, input_pos, input_imgs, img_loc, token_type_ids
        )
