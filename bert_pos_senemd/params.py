...
def read_command_line(argv=None):
    ...
    # -------------------------------------------------------------------------
    # Data input settings
    parser.add_argument('-visdial_processed_train', default='../../data/visdial_0.9_train.json',
                        help='json file containing train split of visdial data')
    parser.add_argument('-visdial_processed_val', default='../../data/visdial_0.9_train.json',
    ...
    parser.add_argument('-visdial_processed_train_dense_annotations',
    ...
    parser.add_argument('-visdial_processed_val_dense_annotations',
                        default='../../data/visdial/visdial_1.0_val_dense_annotations_processed.json',
                        help='JSON file with dense annotations')
    parser.add_argument('-start_path', default='bert-base-uncased-pytorch_model.bin',
                        help='path of starting model checkpt')
    parser.add_argument('-model_config', default='config',
                        help='model definition of the bert model')
    parser.add_argument('-pos_dictionary',default="pos_tag_dictionary",help="path of part of speech tag dictionary file")
    # -------------------------------------------------------------------------
    # Logging settings
    ...
    parser.add_argument('-visdom_server', type=str, default='http://localhost:8097',
    ...
    parser.add_argument('-image_lr', default=3e-5, type=float, help='learning rate for vision params')
    ...
    parser.add_argument('-n_gpus', default=3, type=int, help='number of gpus running the job')
    parser.add_argument('-sequences_per_image', default=8, type=int,
                        help='number of sequences sampled from an image during training')
    parser.add_argument('-visdial_tot_rounds', default=11, type=int,help='number of rounds to use in visdial,caption is counted as a separate round, therefore a maximum of 11 rounds possible')
    parser.add_argument('-max_seq_len', default=256, type=int, help='maximum sequence length for the dialog sequence')
    parser.add_argument('-num_negative_samples', default=1, type=int,
    ...
    parser.add_argument("-pos_loss_coeff", default=1, type=float, help='Coeff for tag masked loss')
