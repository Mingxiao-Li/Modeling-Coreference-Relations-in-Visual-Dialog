...
class VDDataset(data.Dataset):
    ...
    def encode_input(self, utterances, MASK, CLS, SEP, max_seq_len, mask_prob=0.15):
        ...
        assert (
            len(masked_token_list)
            == len(token_id_list)
            == len(pos_tag_target_list)
            == len(pos_tag_list)
            == len(pro_index_list)
        )
    ...
    def __getitem__(self, index):
        ...
        if self._split == "train":
            ...
            for j, context in enumerate(utterances):
                ...
                ## Generating worng order negative sample
                ...
                pos_tag_target_all_rnd.append(
                    torch.cat(pos_tag_target_all, 0).unsqueeze(0)
                )
            ...
            for rnd, cur_rnd_options in enumerate(options_all):
                ...
                pos_tag_target_all.append(
                    torch.cat(pos_tag_target_all_rnd, 0).unsqueeze(0)
                )
            ...
            features, spatials, image_label, image_target, image_mask = self.encode_img(
